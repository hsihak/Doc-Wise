...
def get_highlighted_sentences(file_path_A, file_path_B, threshold):
    ...
    if file_type_A == 'docx':
        ...
        converted_file_A = convert_to_docx(file_path_A)
    ...
    if file_type_B == 'docx':
        ...
        # Convert DOC file to DOCX format
        converted_file_B = convert_to_docx(file_path_B)
        # Load the text from converted DOCX file using python-docx library
...
def highlight_differences(file_path_A, file_path_B, output_file_path_A, output_file_path_B, threshold):
    file_type_A = file_path_A.split('.')[-1].lower()
    file_type_B = file_path_B.split('.')[-1].lower()
...
# Prompt the user to upload the input files
uploaded_A = files.upload()
uploaded_B = files.upload()

# Get the file paths
file_path_A = next(iter(uploaded_A))
file_path_B = next(iter(uploaded_B))
...
def output_paragraphs(output_file_path, docx, highlighted_sentences):
  output = Document()
  in_metadata = True
  for par in docx.paragraphs:
    ...
    if runs: # check that the runs are not empty
      ...
      font_info = read_font_info(runs[0])

    los = split_text_into_sentence_list(par.text) #los stands for list_of_sentences
    ...
    for sent in los:
      ...
      out_run.bold = true_false(font_info['bold'])
      out_run.italic = true_false(font_info['italic'])
      out_run.underline = true_false(font_info['underline'])
      # make sure to account for all forms of sentences (just in case)
      highlight = False
      # account for both header format and sentence format
      if sent_highlighted(sent, highlighted_sentences):
...
def highlight_and_format(file_path_A, file_path_B, output_file_path_A, output_file_path_B, threshold):
  # get the list of sentences that are highlighted
  final_path_A, final_path_B = get_final_paths(file_path_A, file_path_B)
  ...
  output_paragraphs(output_file_path_A, docx_a, highlighted_sentences_A)
  output_paragraphs(output_file_path_B, docx_b, highlighted_sentences_B)

# Testing the document highlighting functionality.
# Setting up file paths for the output highlighted documents.
output_file_path_A = 'formated_highlighted_A.docx'
output_file_path_B = 'formated_highlighted_B.docx'

# Define a user-defined threshold for highlighting differences.
# This threshold determines the level of similarity below which sentences will be highlighted.
threshold = 0.50

# Call the highlight_and_format function to process and highlight differences in the documents.
# The function takes in the file paths of the two documents to be compared, the output file paths,
# and the similarity threshold. It outputs two new documents with the highlighted differences.
highlight_and_format(file_path_A, file_path_B, output_file_path_A, output_file_path_B, threshold)

# Additional test to verify the effectiveness of the highlighting.
# Set up different output file paths and a higher threshold for a more stringent comparison.
output_file_path_A = 'highlighted_job_description_A.docx'
output_file_path_B = 'highlighted_job_description_B.docx'
threshold = 0.80

# Get the final processed paths for the input documents, especially necessary if they are in PDF format.
final_path_A, final_path_B = get_final_paths(file_path_A, file_path_B)

# Re-run the highlighting function with the new threshold and file paths.
# This step is useful to check the robustness of the highlighting mechanism under different thresholds.
highlight_differences(final_path_A, final_path_B, output_file_path_A, output_file_path_B, threshold)
